...
#include "tb4_cpp_prac6/angleutils.hpp"
#include "tb4_cpp_prac6/iar_pf_kdtree.hpp"
...
namespace iar_amcl
{
    ...
    AmclNode::AmclNode(): nav2_util::LifecycleNode("amcl", "", true)
    {
        RCLCPP_INFO(get_logger(), "Creating a IAR AMCL");
        // robot motion parameters
        add_parameter("alpha1", rclcpp::ParameterValue(0.2), "This the alpha1 parameter for robot motion model");
        add_parameter("alpha2", rclcpp::ParameterValue(0.2), "This the alpha2 parameter for robot motion model");
        add_parameter("alpha3", rclcpp::ParameterValue(0.2), "This the alpha3 parameter for robot motion model");
        add_parameter("alpha4", rclcpp::ParameterValue(0.2), "This the alpha4 parameter for robot motion model");
        // robot sensor model parameters
        ...
        add_parameter("sigma_hit", rclcpp::ParameterValue(0.2), "Gaussian variance for correct measurement");
        ...
        /* for the Turtlebot4, the value should be "base_link" as the robot description topic does not include 
        the "base_foot" frame, which is conventional base frame id for NAV2*/
        add_parameter("base_frame_id", rclcpp::ParameterValue(std::string("base_link")), "The frame for robot base");
        add_parameter("global_frame_id", rclcpp::ParameterValue(std::string("map")),
            "The name of the coordinate frame published by the localization system");
        add_parameter("odom_frame_id", rclcpp::ParameterValue(std::string("odom")), "The frame for odometry");
        ...
        // names of topics parameters
        add_parameter("scan_topic", rclcpp::ParameterValue("scan"), "subscribed laser scan topic for localisation");
        add_parameter("map_topic", rclcpp::ParameterValue("map"), "subscribed map topic, where robots are localised");
    ...
    nav2_util::CallbackReturn AmclNode::on_configure(const rclcpp_lifecycle::State &) 
    {
    ...
    void AmclNode::pf_resample(pf_t * pf)
    {
        pf_sample_set_t * old_set, * new_set;
        pf_sample_t * sample_in_old_set, * sample_in_new_set;

        old_set = pf->sets + pf->current_set;
        new_set = pf->sets + (pf->current_set + 1) % 2;


        double * cumulative_pdf_of_old_set;
        cumulative_pdf_of_old_set = (double *)malloc(sizeof(double) * (old_set->sample_count + 1));

        //TODO #1 Calculate the cumulative importance weight here for resampling purpose, and save 
        // to a array "cumulative_pdf_of_old_set"
  
        iar_amcl::pf_kdtree_clear(new_set->kdtree);

        
        //TODO #2 Define and initialise variables you think it is neccessary for TODO #3

        while(new_set->sample_count < pf->max_samples)
        {
            sample_in_new_set = new_set->samples + new_set->sample_count;

            //TODO #3 implenting the Augmented MCL from Step 12 to Step 19
            iar_amcl::pf_kdtree_insert(new_set->kdtree, sample_in_new_set->pose, sample_in_new_set->weight);
            new_set->sample_count++;
        }

        //TODO #4, normalise the sample weights after resampling, to ensure the sum of all weights of all samples
        // is equal to 1

        pf_cluster_stats(pf, new_set);
        pf->current_set = (pf->current_set + 1) % 2;
        free(cumulative_pdf_of_old_set);
    }
    ...
    void AmclNode::handleInitialPose(geometry_msgs::msg::PoseWithCovarianceStamped & msg)
    {
        ...
        geometry_msgs::msg::TransformStamped tx_odom; // define a  transformation between two coordinate frames
        try{
            rclcpp::Time rclcpp_time = now(); //retrieves the current time

            // Here, the rclcpp::Time value is converted to tf2::TimePoint. 
            // The tf2::TimePoint class is used by the tf2 library to represent time.
            ...
            // this line sets the tx_odom.transform to the identity transform. 
            // It uses a converter to convert from "tf2::Transform" to "tx_odom.transform".
            tf2::impl::Converter<false, true>::convert(tf2::Transform::getIdentity(), tx_odom.transform);
        }

        // Update the received pose estimate by integrating the odometry change, 
        // and transform it to global frame
    ...
    void AmclNode::initOdometry(){
        ...
        motion_model_ = std::unique_ptr<nav2_amcl::DifferentialMotionModel>(
            new nav2_amcl::DifferentialMotionModel(alpha1_, alpha2_, alpha3_, alpha4_)
        );
    ...
    nav2_util::CallbackReturn AmclNode::on_deactivate(const rclcpp_lifecycle::State &)
    {
    ...
    nav2_util::CallbackReturn AmclNode::on_cleanup(const rclcpp_lifecycle::State &)
    {
    ...
    nav2_util::CallbackReturn AmclNode::on_shutdown(const rclcpp_lifecycle::State &)
    {
        RCLCPP_INFO(get_logger(), "Shutting down");
        return nav2_util::CallbackReturn::SUCCESS;
    }

    // void AmclNode::iar_pf_kdtree_insert(pf_kdtree_t * self, pf_vector_t pose, double value){
    //       int key[3];
    ...
    // }
    // void AmclNode::iar_pf_kdtree_clear(pf_kdtree_t * self){
    //     self->root = NULL;
    //     self->leaf_count = 0;
    //     self->node_count = 0;
    // }
